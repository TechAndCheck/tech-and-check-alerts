...
class AbstractNewsletter {
   ...
   * `getIsNewsletterSendable()` on the extending class and simply return true.
   *
  ...
  getIsNewsletterSendable = async () => {
    throw new Error('You extended AbstractNewsletter but forgot to define getIsNewsletterSendable()')
  }
   ...
   */
  send = async () => {
    const isNewsletterSendable = await this.getIsNewsletterSendable()
    if (!isNewsletterSendable) {
      return Promise.reject(new Error('The newsletter did not satisfy its sendability requirements.'))
    }
